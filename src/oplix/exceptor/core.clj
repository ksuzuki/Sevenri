...
(ns oplix.exceptor.core
  (:use [openar config log oplix utils]
        [oplix.exceptor defs edb refs])
  (:import (java.io File)))
...
(defn launch-exceptor
  [#^Exception e fqon file-name line-number]
  #_(lg "fqon:" fqon "file-name:" file-name "line-number:" line-number)
  (if (re-find (re-pattern (str (get-oplix-fqns *ced*))) (str fqon))
    ;; Cannot open ced when it's the cause of the exception.
    (do-launch e (File. file-name) line-number false)
    (let [sub-path (nssym2path fqon)
