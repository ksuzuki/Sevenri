...
(defn add-ui
  ...
  (let [frm (oplix-frame)
    ...
    (put-oplix-prop *oplix* :panel pnl)
    (doto tpn
...
(defn add-toolboxes
  []
  (let [pnl (get-oplix-prop :panel)]
    (doseq [[box pos] (get-oplix-prop :boxes)]
      (.add pnl box pos))
    (.validate pnl)))

(defn remove-toolboxes
  []
  (doseq [[box _] (get-oplix-prop :boxes)]
    (.remove (get-oplix-prop :panel) box)))
...
(defn add-panel-ui
  ...
  (let [frm (oplix-frame)
    ...
    ;;
    (put-oplix-prop *oplix* :panel pnl :boxes [[bx1 BorderLayout/NORTH]
                                               [bx2 BorderLayout/SOUTH]])
    (add-toolboxes)))

(defn update-title
  [section]
  (let [title (str (oplix-name) (when section (str " - " section)))]
    (set-oplix-title title)))
...
(defn get-control
  ([storage ctrl-kwd]
  (if (is-oplix? storage)
    (get-control storage ctrl-kwd (get-oplix-prop storage :panel))
