...
(ns oplix.incantea.init
  (:use [openar config event log oplix ui]
        [oplix.incantea core defs listeners ui])
...
(defn save-spell-name
  ...
  (with-create-on-get-dir
    (let [sf (get-dop-oplix-state-file)]
...
(defn restore-ui
  ...
  (let [frame (oplix-frame)
    ...
    ;;
    (add-incant-listener incnt splls *oplix*)
    (add-edit-listener edspl splls)
    (add-repl-listener srepl (oplix-name) get-incantea-repl-name)
...
(defn save-incantea
  [event]
  (if (:openar.event/info-save-on-close (get-event-info event))
    (do
      (save-state)
      (do-views :close-all-yes)
      (create-event-response
       :openar.event/response-suppress-xml-encoder-errors))
    (create-event-response
     :openar.event/response-donot-save)))
