...
(ns oplix.ced.listeners
  (:use [openar log]
        [openar.oplix :only (invoke-later)]
        [oplix.ced defs indent ui]
        [library.oplix.ced.paren :only (find-matching-paren)])
             ...
             (when (future-done? fmp)
               (let [[new-ppp fmpparam] @fmp
                 ...
                 (when (or (not= old-ppp new-ppp) (not= old-clear? clear?))
                   ...
                   (invoke-later (.getClientProperty ced *prop-ced-oplix*)
