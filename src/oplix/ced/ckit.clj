...
(ns oplix.ced.ckit
  ...
  (:use [openar config core log oplix utils]
        [oplix.ced defs listeners indent utils]
        [oplix.ced.find :only (find-context-namespace)]
        [oplix.ced.ui :only (popup-warning)]
  ...
  (let [beg (.getSelectionStart ced)
    ...
    (when (< bli eli)
      (let [doc (get-doc-context :doc)
            ...
            oplt (.getClientProperty ced *prop-ced-oplix*)]
        (future
      ...
      (ced-action event
        (let [api-sym (or (.getSelectedText ced)
          ...
          (when-not (empty? api-sym)
            (future (open-oplix-with-args {:keyword api-sym} 'api-browser))))))))
...
(defn invoke-opfile-with
  ...
  (let [doc (.getDocument ced)
    ...
    (if-let [mtch (re-matches (re-pattern (str "^" (get-src-dir) "/(.*)\\.clj$")) (str file))]
      (let [fqon (path2nssym (second mtch))
            opon (when-let [m (re-matches (re-pattern (str "^" (get-default :on :oplix) "\\.(.*)$")) (str fqon))]
                   (second m))
            fons (filter #(re-matches (re-pattern (str "^" % "(\\..*)*")) (str opon)) (get-all-oplix-on))
            on (when (seq fons)
                 (reduce (fn [l n] (if (< (count (str l)) (count (str n))) n l)) nil fons))]
        #_(lg "invoke-opfile-with fqon:" fqon "opon:" opon "on:" on)
        (action-fn fqon on))
      (popup-warning (.getTopLevelAncestor ced) "Not an openar/oplix file"))))
          ...
          (fn [fqon on]
            (future
              (let [ct (Thread/currentThread)
                ...
                (try
                  (.setContextClassLoader ct (if on
                                               (create-oplix-class-loader on)
                                               ccl))
                  (require fqon :reload)
                  (catch Exception e
                    (log-exception e fqon))
                  (finally
      ...
      (ced-action event
        (invoke-opfile-with ced
          (fn [fqon on]
            (future
              (try
                (binding [*compile-path* (str (get-dop-classes-dir))]
                  (compile fqon))
                (catch Exception e
                  (log-exception e fqon))))))))))

(defn open-oplix-action
  []
  (proxy [AbstractAction] ["open-oplix"]
    (actionPerformed
      [event]
      (ced-action event
        (invoke-opfile-with ced
    ...
    (actionPerformed
      ...
      (ced-action event
        (let [frm (.getTopLevelAncestor ced)
              dlg (FileDialog. frm "Save As Openar File" FileDialog/SAVE)
              doc (.getDocument ced)
              fpt (.getFile doc)
          ...
          (when-let [f (.getFile dlg)]
            (let [f-clj (if-let [m (re-find rpt f)]
              ...
              (if save?
                (do
                  (.saveAs doc fpath)
                  (when-let [oplt (.getClientProperty ced *prop-ced-oplix*)]
...
(defn open-action
  ...
  (proxy [AbstractAction] ["open"]
    (actionPerformed
      ...
      (ced-action event
        (let [frm (.getTopLevelAncestor ced)
              dlg (FileDialog. frm "Open Openar File" FileDialog/LOAD)
              doc (.getDocument ced)
              fpt (.getFile doc)
              dir (if (instance? File fpt)
                    (.getParent fpt)
          ...
          (when-let [f (.getFile dlg)]
            (open-oplix-with-args {:file (File. (.getDirectory dlg) f)} 'ced)))))))
