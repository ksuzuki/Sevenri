...
(defn get-file
  "Return a File object specified by a file specifier. The specifer is
   ...
   are applied to it and then a file in the src or src/oplix directories is
   ...
   file doesn't exist."
  ([]
     (get-file (:file (oplix-args))))
  ([afile]
     (let [rptn (re-pattern
                 (str "^(" (apply str (butlast (interleave (get-openar-namespaces) (repeat \|)))) ")[./].*"))
           fpath (cond
                  (symbol? afile) (let [file (str afile)]
                                    (if (re-matches rptn file)
                                      (File. (get-src-dir) (nssym2path file))
                                      (File. (get-oplix-dir) (nssym2path file))))
...
(defn update-title
  "Should be called in the *oplix* context."
  [doc]
  (add-to-xref *oplix* :ced-file (.getFile doc))
  (let [fname (.getFileName doc)
        fnlen (count fname)
        fnstr (.substring fname 0 (if (re-find #"\.clj$" fname) (- fnlen 4) fnlen))
        title (str (oplix-name) " - " (path2nssym fnstr))]
