...
(ns slix.incantea.listeners
  (:use [sevenri config core log slix ui utils]
        [slix.incantea core defs])
...
(defn show-spell-in-title
  [list]
  (when-let [slix (get-slix (.getTopLevelAncestor list))]
    (let [spell (.getSelectedItem list)
          sfile (get-spell-file spell)]
      (add-to-xref slix :incantea-file sfile)
      (set-slix-title slix (str (slix-name slix)
  ...
  (when-let [spl (.getSelectedItem list)]
    (let [frm (.getTopLevelAncestor actions)
      ...
      (when (= ync JOptionPane/YES_OPTION)
        (let [slix (get-slix frm)
              sfile (get-spell-file spl)
              opkys (seq (filter (fn [[o k]] (not (identical? o slix))) (xref-with sfile)))]
          (if (seq opkys)
...
(defn add-repl-listener
  [button slix-name get-repl-name-fn]
  (remove-listeners button #(.getActionListeners %) #(.removeActionListener %1 %2))
  (.addActionListener
   button
   (proxy [ActionListener] []
     (actionPerformed [e]
       (when-let [rn (get-repl-name-fn slix-name)]
         (let [ro (get-slix rn)]
           (if ro
             (.toFront (slix-frame ro))
             (open-slix-with-args slix-name 'repl rn))))
       (.requestFocusInWindow button)))))
