...
(ns slix.documenter.listeners
  (:use [clojure.java browse]
        [sevenri log slix]
        [slix.documenter md2html mddb io ui])
...
(defn in-edit
  ([frame]
     (let [slix (get-slix frame)]
       (:documenter-in-edit (xref-with slix))))
  ([frame stcvec]
     (let [slix (get-slix frame)
           stcvc (when (vector? stcvec) stcvec)]
       (add-to-xref slix :documenter-in-edit stcvc)
       (invoke-later slix
           ...
           (if (and (contains?? (get-sections title ctgry) sectn)
             ...
             (let [sectx (:section-text ctrls)
               ...
               (when-let [ovs (in-edit-by frame stcvc)]
                 ...
                 ;; Then bring the slix which has editable text pane
                 ;; to the font.
                 ...
                 (when-first [ov (filter (fn [[o v]]
                                         ...
                                         ovs)]
                   (invoke-later (first ov) #(.toFront (slix-frame (first ov)))))))
             (do
               (in-edit frame nil)
               (disable-controls (vals ctrls))
               (enable-controls true [clist cacts])
