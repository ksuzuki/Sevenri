...
(ns openar.ui
  (:use [openar config defs event log refs])
...
(using-fns ui oplix
           [is-oplix? oplix-frame
            put-oplix-prop get-oplix-prop remove-oplix-prop
            get-oplixes close-oplix get-oplix-openar])
...
(defn update-z-order
  ([]
     (let [live-frames-set (apply hash-set (map #(ui-using-oplix-frame-oplix %) (ui-using-get-oplixes-oplix)))
...
(defn default-close-window
  [frame]
  (let [fo (filter #(= frame (ui-using-oplix-frame-oplix %)) (ui-using-get-oplixes-oplix))]
    (if-let [oplix (first fo)]
      (ui-using-close-oplix-oplix oplix)
      (.dispose frame))))
...
(defn add-default-window-listener
  [frame]
  (let [dwlc (Class/forName (str (get-default :src :openar :listeners :defwinlistener)))
...
(defn add-default-key-listener
  [comp]
  (let [dklc (Class/forName (str (get-default :src :openar :listeners :defkeylistener)))
...
(defn clear-saved-dynaclass-listeners
  [oplix]
  (ui-using-remove-oplix-prop-oplix oplix *saved-dynaclass-listeners*))

(defn restore-saved-dynaclass-listeners
  "[ [comp [[adder dc-listeners]*]* ]"
  [oplix]
  (when-let [calv-vec (ui-using-get-oplix-prop-oplix oplix *saved-dynaclass-listeners*)]
    (when (seq calv-vec)
...
(defn save-dynaclass-listeners
  ...
  [oplix cagrv-vec]
  (when (ui-using-is-oplix?-oplix oplix)
    (let [calv-vec (reduce (fn [calvv [comp agrvv]]
                           ...
                           cagrv-vec)]
      (when (seq calv-vec)
        (ui-using-put-oplix-prop-oplix oplix *saved-dynaclass-listeners* calv-vec)))))
...
(defmacro save-dyna-listeners
  ([cagrv-vec]
     `(save-dynaclass-listeners ~'*oplix* ~cagrv-vec))
  ([oplix cagrv-vec]
     `(save-dynaclass-listeners ~oplix ~cagrv-vec)))
     ...
     (proxy [AWTEventListener] []
       (eventDispatched [event]
         (when (and (instance? JFrame (.getWindow event))
                    ...
                    (not= (get-last-global-event) :openar.event/openar-quitting))
           (let [ops (ui-using-get-oplixes-oplix)]
             (cond
              (= (count ops) 1)
              (let [frame (ui-using-oplix-frame-oplix (first ops))]
                (EventQueue/invokeLater #(.toFront frame)))
              (every? true? (map #(pos? (bit-and (.getExtendedState (ui-using-oplix-frame-oplix %)) JFrame/ICONIFIED)) ops))
              (let [openar (ui-using-get-oplix-openar-oplix)]
                    (.toFront (ui-using-oplix-frame-oplix (or openar (first ops))))))))))
