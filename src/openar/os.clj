...
(ns openar.os
  (:use [openar config defs log]
        [openar.defs :only (*ok-to-quit-fn*)])
  (:import (java.io File)
...
(defn- set-mac-dock-icon
  ...
  (let [fp (File. (get-resources-dir (get-default :src :resources :images :dir-name)
                  ...
                  (str (get-default :src :resources :images :icons :openar-icon-file-name)))]
    (when (.exists fp)
...
(defn- -remove-mac-application-listener
  "Do this before saving an oplix frame or addWindowListner call with
...
(defn presave-oplix-frame-os
  [frame]
  (cond
   (is-mac?) (-remove-mac-application-listener frame)
   :else nil))

(defn postsave-oplix-frame-os
  [frame presave-oplix-os-value]
  (cond
   (is-mac?) (-add-mac-application-listener frame presave-oplix-os-value)
   :else nil))
